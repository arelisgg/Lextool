Vue.component('lex-article-element', {
    template:
        ...
        <li class="lex_element" @click="updateElement(element.lex_article_element.id_lex_article_element)">
              <span style="margin-left: 3px" v-if="element.sub_model_separator !== null && element.sub_model_separator !== undefined && element.sub_model_separator !== '' && isFirstElement">{{ element.sub_model_separator.representation }}</span>
            
              <span v-if="hasVisibility" class="elem" :style="elementStyles">
                {{ element.lex_article_element.element }}
                  <span v-if="element.element_separator !== null && element.element_separator !== undefined" class="sep">{{ element.element_separator.representation }}</span>
              </span>
              <span class="sub" v-if="subelementHasVisibility" :style="subElementStyles"> 
                 {{ element.sub_element_type }}
                 <span v-if="element.element_separator !== null && element.element_separator !== undefined" class="sep">{{ element.element_separator.representation }}</span>
              </span>
              <span v-if="element.element.property !== 'Lema-entrada'" class="elem-remover fa fa-remove" @click.self="deleteElement(element.lex_article_element.id_lex_article_element)">
              </span>
         </li>
         `,
    props: {
    ...
    computed: {
        ...
        isFirstElement: function () {
            if ((this.element.sub_model_separator !== undefined && this.element.sub_model_separator !== null && this.element.sub_model_separator !== '')
                && this.element.sub_model_active.elements !== undefined)  {
